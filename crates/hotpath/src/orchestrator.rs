...
use crate::{
    config_atomic::{ParamVec, AtomicConfig, ConfigSnapshot},
    telemetry::TelemetryDigest,
    spsa::{Spsa, SpsaConfig, SpsaState},
    proposer::{Proposal, ProposalResult, ProposalError, NoChangeReason, AdaptiveProposer},
    executor::{SafeExecutor, SafetyExecutor, Guardrails, ApplyReceipt, Violation},
};
use std::sync::Arc;
...
impl AdaptiveProposer for SpsaProposer {
            ...
            SpsaState::WaitingPlus { .. } => {
                ...
                if self.spsa.has_enough_samples() {
                    ...
                    if let Some(ref delta) = self.current_delta {
                        ...
                        Err(ProposalError::InternalError("No delta available".to_string()))
                    }
                } else {
    ...
    fn current_perturbation(&self) -> Option<(u64, ParamVec)> {
        self.current_delta.clone().map(|d| (self.spsa.perturbation_counter(), d))
    }
